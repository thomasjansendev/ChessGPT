...
def main():
    ...
    while running and not board.checkmate and board.halfmove_clock <= 50:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                for piece in board.active_pieces[board.active_colour]:
                    if piece.rect.collidepoint(event.pos):
                        grabbed_piece = piece
                        grabbed_piece.rect.center = (
                            event.pos[0],
                            event.pos[1],
                        )  # snap piece to mouse
                        origin_square = idx_to_name(
                            piece.get_position(board.array)
                        )  # calling negates the performance improvement of looping through active pieces of current player -> consider changing this
                        origin_rect = board.sprites[origin_square]["rect"]
                        possible_moves = grabbed_piece.get_legal_moves(
                            board
                        )  # called here and in board.update() <- TODO: make this better
                        dragging = True
                        break
                ...
                for square in possible_moves:
                    if board.sprites[square]["rect"].collidepoint(event.pos):
                        ...
                        try:
                            board.update(
                                user_move
                            )  # In theory, this should never raise an exception since we only loop through possible moves
                ...
                if (
                    not succesful_move
                ):  # if move is not valid then reset position of piece to origin
        ...
        if possible_moves != None:
            for square in possible_moves:
                pygame.draw.circle(
                    screen, (0, 0, 0), board.sprites[square]["rect"].center, 10
                )
