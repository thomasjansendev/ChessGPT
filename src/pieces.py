...
class Piece:
    ...
    def get_legal_moves(
        self, board, position: tuple = None
    ) -> list:  # TODO: (perf) add position as method parameter
        if position == None:  # temp check until position parameter is fully utilized
            position = self.get_position(board.array)
        possible_moves = move_search(
            board.array, position, self, mode="-legal", format="-list"
        )
        possible_moves = filter_moves_leading_to_check(
            board, self, position, possible_moves
        )
    ...
    def get_attacking_squares(self, board) -> list:
        position = self.get_position(board.array)
        possible_moves = move_search(
            board.array, position, self, mode="-attacking", format="-list"
        )
        return possible_moves
...
class King(Piece):  # can move to any adjacent square by 1 => 8 DOF
    ...
    def get_legal_moves(self, board, position=None) -> list:
        ...
        legal_moves = filter_moves_leading_to_check(
            board, self, position, possible_moves
        )
        legal_moves = append_castling_moves(
            self, board, squares_under_threat, legal_moves
        )
        return legal_moves
...
class Pawn(Piece):  # 1.5 DOF
    ...
    def get_legal_moves(self, board, position=None) -> list:
        ...
        possible_moves_dict = move_search(
            board.array, position, self, mode="-legal", format="-dict"
        )
        possible_moves_list = []
        ...
        for i in [
            1,
            2,
        ]:  # <- these correspond to the indices of the diagonals in self.moveset
        ...
        possible_moves_list = filter_moves_leading_to_check(
            board, self, position, possible_moves_list
        )
    ...
    def get_attacking_squares(self, board) -> list:
        ...
        possible_moves_dict = move_search(
            board.array, position, self, mode="-attacking", format="-dict"
        )

        diagonal_moves = (
            possible_moves_dict[self.moveset[1]] + possible_moves_dict[self.moveset[2]]
        )
...
def move_search(
    ...
    if depth == None:
        depth = len(BOARD_REF)  # default to searching whole array
    elif depth < len(BOARD_REF):
        depth += 1  # +1 because range() excludes the upper bound
    elif depth >= len(BOARD_REF):
        depth = len(BOARD_REF)  # caping the value of depth
    ...
    for direction in moveset:
        ...
        for i in range(1, depth):
            ...
            search_pos = (
                origin[0] + i * ARRAY_CARDINALS[direction][0],
            ...
            if (
                search_pos[0] < 0
                or search_pos[0] >= len(board)
                or search_pos[1] < 0
                or search_pos[1] >= len(board)
            ):
                break
    ...
    # Return list as default
        ...
        raise Exception(
            "move_search: 'format' argument is invalid. Should either be '-dict' or '-list'"
        )
...
def get_squares_under_threat(board):
    ...
    result = list(
        set(squares_under_threat)
    )  # convert to set to remove duplicate values

    return result


def filter_moves_leading_to_check(
    board, piece: Piece, origin_square_idx: str, possible_moves: list
):
...
def append_castling_moves(
    king: King, board, squares_under_threat: list, legal_moves: list
) -> list:
    queenside_moves = []
    kingside_moves = []
    ...
    if king.colour == "w":
        ...
        if board.castling_availability["white_queenside"] == False:
            ...
            board.get_piece("b1") != None
            or board.get_piece("c1") != None
            or board.get_piece("d1") != None
        ):
            pass
        else:
        ...
        if board.castling_availability["black_queenside"] == False:
            ...
            board.get_piece("b8") != None
            or board.get_piece("c8") != None
            or board.get_piece("d8") != None
        ):
            pass
        else:
